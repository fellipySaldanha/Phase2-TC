...
export default class CustomerController {
  ...
  async getCustomers(queryParams: ParsedQs, response: Response): Promise<any> {
    try {
      if (queryParams.id) {
        const result = await this.repository.getCustomerById(
          Number(queryParams.id),
        );
        return response.status(200).json(result);
      }
      if (queryParams.cpf) {
        const result = await this.repository.getCustomerByCPF(
          Number(queryParams.cpf),
        );
        return response.status(200).json(result);
      }
  ...
  async deleteCustomer(
    ...
    try {
      const result = await this.repository.deleteCustomer(
        Number(queryParams.id),
      );
      if (result?.affectedRows > 0) {
        response
          .status(200)
          .json({ Success: `Row with Id ${queryParams.id} deleted` });
      } else {
