...
import { MoveNextInputDTO, MoveNextOutputDTO, orderqueueInfo } from "./MoveNextDTO";

export class MoveNextUseCase {

    static async execute(params: MoveNextInputDTO, gateway: IOrderQueueGateway): Promise<MoveNextOutputDTO> {
        try {
            ...
            await gateway.updateOrderQueue(params.id, status_queue_enum_id, waiting_time);
            gateway.commit();
